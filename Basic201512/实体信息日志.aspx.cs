...
public partial class Basic201512_实体信息日志 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            ViewState["init"] = "select * from e_log where Message like'%捐助人%'or Message like'%受助人%' or Message like '%经办单位%' or Message like '%信息%'  or Message like '%用户%' order by ID DESC";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if(user=="")
        {
            ...
            {
                ViewState["now"] = "select * from e_log where  Message like'%捐助人%'or Message like'%受助人%' order by ID DESC";
                databind(ViewState["now"].ToString());
                return;
            }
            if(TextBox2.Text.ToString().Length!=0&& TextBox3.Text.ToString().Length == 0)
            {
                ...
                ViewState["now"] = "select * from e_log" + temp + "and Message like'%捐助人%'or Message like'%受助人%' order by ID DESC";
                databind(ViewState["now"].ToString());
                return;
            }
            if (TextBox2.Text.ToString().Length == 0 && TextBox3.Text.ToString().Length != 0)
        ...
        else//user!=""
        {
            ...
            if (TextBox2.Text.ToString().Length == 0 && TextBox3.Text.ToString().Length == 0)
            {
                ViewState["now"] = "select * from e_log" + temp + "and Message like'%受助人%'or Message like'%捐助人%' order by ID DESC";
                databind(ViewState["now"].ToString());
                return;
            }
            else
